...
local CORNER = 12 -- Eckenradius der Kacheln, angelehnt an macOS-Fenster
...
function overview.show()
    ...
    -- Ohne Bedienungshilfen reagiert kein Klick. Der Hotkey selbst
    -- funktioniert trotzdem, deshalb der Hinweis an dieser Stelle.
    -- hs.urlevent.openURL() verlangt zwingend ein "://" in der URL und
    -- lehnt Apples eigenes Schema ab, deshalb hier /usr/bin/open.
    local axOk, axGranted = pcall(hs.accessibilityState)
    if axOk and axGranted == false then
        hs.alert.show(
            "Quitly braucht die Bedienungshilfen.\n" ..
            "Setze in den Systemeinstellungen den Haken bei Hammerspoon.", 5)
        hs.execute(
    ...
    for i, w in ipairs(windows) do
        ...
        local frame = { x = x, y = y, w = THUMB_W, h = THUMB_H }
        local radii = { xRadius = CORNER, yRadius = CORNER }

        -- Karte hinter dem Thumbnail. Sie gibt der Kachel die runden Ecken
        -- und den weichen Schatten, auch wenn das Fenster ein anderes
        -- Seitenverhältnis hat als die Kachel.
        c[#c + 1] = {
            type = "rectangle",
            action = "fill",
        ...
        -- Alles Folgende auf die runde Karte beschneiden
        c[#c + 1] = {
            type = "rectangle",
            action = "clip",
            roundedRectRadii = radii,
        ...
        -- Dezenter Rahmen, ebenfalls mit runden Ecken
        c[#c + 1] = {
            type = "rectangle",
            action = "stroke",
