...
OUT = os.path.join(REPO, "dist", "Quitly-%s.pkg" % VERSION)
...
def main():
    ...
    for name in ("window_overview.lua", "welcome.lua", "init.lua", "uninstall.sh"):
        shutil.copy2(os.path.join(SRC, "payload", name), os.path.join(support, name))
    ...
    for name in ("preinstall", "postinstall"):
        ...
        shutil.copy2(os.path.join(SRC, "scripts", name), target)
...
<installer-gui-script minSpecVersion="1">
    <title>Quitly</title>
    <organization>com.github.jotree96</organization>
...
""".format(ident=IDENTIFIER, ver=VERSION, kb=kbytes, pkgdir=PKG_DIR_NAME)
    ...
    for name in ("welcome.html", "conclusion.html"):
        shutil.copy2(os.path.join(SRC, "resources", name), os.path.join(resources, name))
